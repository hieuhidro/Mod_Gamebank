...
class payment {
	...
	public function insertItemp($username = null) {
		...
		if ($username != null) {
			...
			$table_name = $options->gamebank_prefix.'payment_history';
...
class payment_history {
	...
	public function getItemFilter($cardserial = "",$status = 0,$from = 0,$limit = 20){
		...
		$table_name = $options->gamebank_prefix. 'payment_history';
		if($status != 0){			
			$sql = "select historyid, datetime,cardserial,cardnumber,coins,status from $table_name where username = '".$this->username."' and cardserial like '%$cardserial%' and status = '$status' limit $from, $limit";
		}else{
