...
/*======================================================================*\
|| #################################################################### ||
|| # vBulletin 4.1.5 - Null_by_vietvbb_team
|| # ---------------------------------------------------------------- # ||
...
if ($_REQUEST['do'] == 'transaction')
{
if($GLOBALS['vbulletin']->options['gbservicesonoff']){
	($hook = vBulletinHook::fetch_hook('profile_editpassword_start')) ? eval($hook) : false;
	...
	$page_templater = vB_Template::create('gbcard_form');
	...
	if(isset($_POST['ok'])){
		...
		if($string==$userstring){
			...
			$code = $_POST['txtCode'];
			if($_POST['txtSeri']!=NULL){
				$seri = $_POST['txtSeri'];
			}else{
				$seri = 'Ch&#432;a nh&#7853;p seri';
			}
			$gamebank_account = "trongsau96";
			if($GLOBALS['vbulletin']->options['gbaccount']!=NULL){
				$gamebank_account=$GLOBALS['vbulletin']->options['gbaccount'];
			}
			$status=0;
			
			$gamebank_url='http://pay.gamebank.vn/service/cardServiceV2.php?wsdl';
			if($GLOBALS['vbulletin']->options['gburl']!=NULL){
				$gamebank_url=$GLOBALS['vbulletin']->options['gburl'];
			}
			$client = new nusoap_client($gamebank_url,true);
			...
			if($result[0] >= 10000)
			{
				if($result[0]==10000 && ($GLOBALS['vbulletin']->options['gbcardtype10']!=NULL)){
					$money=$GLOBALS['vbulletin']->options['gbcardtype10'];
				}elseif($result[0]==20000 && ($GLOBALS['vbulletin']->options['gbcardtype20']!=NULL)){
					$money=$GLOBALS['vbulletin']->options['gbcardtype20'];
				}elseif($result[0]==50000 && ($GLOBALS['vbulletin']->options['gbcardtype50']!=NULL)){
					$money=$GLOBALS['vbulletin']->options['gbcardtype50'];
				}elseif($result[0]==100000 && ($GLOBALS['vbulletin']->options['gbcardtype100']!=NULL)){
					$money=$GLOBALS['vbulletin']->options['gbcardtype100'];
				}elseif($result[0]==200000 && ($GLOBALS['vbulletin']->options['gbcardtype200']!=NULL)){
					$money=$GLOBALS['vbulletin']->options['gbcardtype200'];
				}elseif($result[0]==500000 && ($GLOBALS['vbulletin']->options['gbcardtype500']!=NULL)){
					$money=$GLOBALS['vbulletin']->options['gbcardtype500'];
				}else{
					$money=$result[0];
				}
				
				$gb_credit='user_money';
				if($GLOBALS['vbulletin']->options['gbcredit']!=NULL){
					$gb_credit=$GLOBALS['vbulletin']->options['gbcredit'];
				}
				$status=1;
				$message= "N&#7841;p th&#224;nh c&#244;ng ".$result[0];
				$message2=$result[0];
        $old_money=$db->query_read("SELECT $gb_credit FROM user WHERE userid=".$vbulletin->userinfo['userid']);
        //var_dump($money);
        $moneys=$db->fetch_array($old_money);
        //var_dump($moneys);
        $new_money=$moneys[$gb_credit]+$money;
         //var_dump("UPDATE user SET user_money =$new_money WHERE userid =".$vbulletin->userinfo['userid']);die;
        $db->query_write("UPDATE user SET $gb_credit ='$new_money' WHERE userid =".$vbulletin->userinfo['userid']);

			}
			else
			{
				//L?i n?p ti?n, d?a vào b?ng mã l?i d? show thông tin khách hàng lên
				if($result[0]==-3){
					$message="Th&#7867; kh&#244;ng s&#7917; d&#7909;ng &#273;&#432;&#7907;c";
				}elseif($result[0]==-10){
					$message="Nh&#7853;p sai &#273;&#7883;nh d&#7841;ng th&#7867;";
				}elseif($result[0]==-1001){
					$message="Nh&#7853;p sai qu&#225; ba l&#7847;n";
				}elseif($result[0]==-1002){
					$message="L&#7893;i h&#7879; th&#7889;ng";
				}elseif($result[0]==-1003){
					$message="IP kh&#244;ng &#273;&#432;&#7907;c ph&#233;p truy c&#7853;p";
				}elseif($result[0]==-1004){
					$message="T&#234;n &#273;&#259;ng nh&#7853;p GameBank kh&#244;ng &#273;&#250;ng";
				}elseif($result[0]==-1005){
					$message="Lo&#7841;i th&#7867; kh&#244;ng &#273;&#250;ng";
				}elseif($result[0]==-1006){
					$message="H&#7879; th&#7889;ng &#273;ang &#273;&#432;&#7907;c b&#7843;o tr&#236;";
				}else{
					$message='Kh&#244;ng t&#236;m th&#7845;y l&#7893;i';
				}
				$message2=$result[0];
			}
			//die($telco);
			...
			$date=time();
			$db->query_write("
				INSERT INTO user_gbcard (userid,username,seri,nametype,sale_date,status,gb_tran_id,money)
				VALUES ('$uid','$username','$seri','$nametype',$date,'$message2','$tran_id',$money)"
			);
			$page_templater->register('message', $message);
			}else{
				$message="Vui l&#242;ng nh&#7853;p ch&#237;nh x&#225;c m&#227; x&#225;c nh&#7853;n";
				$page_templater->register('message', $message);
		}
...
if ($_REQUEST['do'] == 'histransaction')
{
	...
	$historytransaction = $db->query_read("
				SELECT * FROM user_gbcard
				WHERE userid =". $vbulletin->userinfo['userid']."
				ORDER BY trid DESC
				LIMIT $current_page,$num_per_page
				"
				);
	$his="<table width='100%' style='margin-top:10px;'>";
	$his.="<tr><td><b>Seri</b></td>";
	$his.="<td><b>Lo&#7841;i th&#7867;</b></td>";
	$his.="<td><b>M&#7879;nh gi&#225;</b></td>";
	$his.="<td><b>Ng&#224;y mua</b></td>";
	$his.="<td><b>Tr&#7841;ng th&#225;i</b></td></tr>";
	date_default_timezone_set("Asia/Ho_Chi_Minh");
	if(mysql_num_rows($historytransaction)){
		while ($history = $db->fetch_array($historytransaction)){
		if($history['status']==-3){
			$history['status']="Th&#7867; kh&#244;ng s&#7917; d&#7909;ng &#273;&#432;&#7907;c";
		}elseif($history['status']==-10){
			$history['status']="Nh&#7853;p sai &#273;&#7883;nh d&#7841;ng th&#7867;";
		}elseif($history['status']==-1001){
			$history['status']="Nh&#7853;p sai qu&#225; ba l&#7847;n";
		}elseif($history['status']==-1002){
			$history['status']="L&#7893;i h&#7879; th&#7889;ng";
		}elseif($history['status']==-1003){
			$history['status']="IP kh&#244;ng &#273;&#432;&#7907;c ph&#233;p truy c&#7853;p";
		}elseif($history['status']==-1004){
			$history['status']="T&#234;n &#273;&#259;ng nh&#7853;p GameBank kh&#244;ng &#273;&#250;ng";
		}elseif($history['status']==-1005){
			$history['status']="Lo&#7841;i th&#7867; kh&#244;ng &#273;&#250;ng";
		}elseif($history['status']==-1006){
			$history['status']="H&#7879; th&#7889;ng &#273;ang &#273;&#432;&#7907;c b&#7843;o tr&#236;";
		}else{
		  $history['status']="Th&#224;nh c&#244;ng";
		}
			
			$his.="<tr><td>".$history['seri']."</td>";
			$his.="<td>".$history['nametype']."</td>";
			$his.="<td>".$history['money']."</td>";
			$his.="<td>".date("d/m/Y h:i:s",$history['sale_date'])."</td>";
			$his.="<td>".$history['status']."</td></tr>";
		}
		$his.= "<tr><td colspan='5' align=center>";
		for($i=1;$i<=$totalpage;$i++){
			if($i!=($current_page/$num_per_page)+1){
				$his.= "<a href=\"gamebank_card_services.php?do=histransaction&page=$i\">".$i."</a> ";
			}else{
				$his.=$i." ";
			}
		}
		...
		$his.="<tr><td cols='5'>Ch&#432;a c&#243; giao d&#7883;ch n&#224;o &#273;&#432;&#7907;c th&#7921;c hi&#7879;n</td></tr>";
	}
	$his.="</table>";
	// don't let banned people edit their email (see bug 2142)
	...
	// don't show optional because password expired
	//$show['password_optional'] = !$show['passwordexpired'];
	$page_templater = vB_Template::create('gbcard_history');
	//$templater = vB_Template::create('modifypassword3');
	$page_templater->register('his', $his);
...
/*======================================================================*\
|| ####################################################################
|| # Downloaded: Null_by_vietvbb_team
